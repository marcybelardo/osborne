...
export const Route = createFileRoute('/_authenticated/budgets/$budgetId/edit')({
...
function EditBudget() {
  ...
  useEffect(() => {
    if (budgetQuery.data) {
      ...
      setDescription(budgetQuery.data.description ?? '')
      setTimeframe(budgetQuery.data.timeframe ?? 'CUSTOM')
      setStartDate(budgetQuery.data.startDate ?? '')
      setEndDate(budgetQuery.data.endDate ?? '')
      setAmount(String(budgetQuery.data.amount))
    }
  }, [budgetQuery.data])
  ...
  const updateMutation = useMutation({
    mutationFn: (data: {
      ...
      apiClient<BudgetResponse>(`/api/budgets/${budgetId}`, {
        method: 'PUT',
        body: data,
      }),
    onSuccess: () => {
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-8">
      ...
      <p className="mt-1 text-sm text-gray-500">Update the budget details</p>
